...
class UpdaterService:
    ...
    def install_update_silently(self, installer_path: str, target_version: str = "") -> Dict:
        ...
        if app_path.suffix.lower() != ".exe":
            target_app = app_path
        handoff.write_text(
            self.handoff_script(installer, str(target_app), target_app.stem, target_version),
            encoding="utf-8",
        )
        try:
            flags = 0
...
function Repair-WhisperwoodShortcuts {{
    ...
    try {{
        ...
        foreach ($base in $bases) {{
            ...
            Get-ChildItem -LiteralPath $base -Recurse -Filter *.lnk -ErrorAction SilentlyContinue | ForEach-Object {{
                $shortcut = $shell.CreateShortcut($_.FullName)
                $name = $_.Name
                $oldTarget = [string]$shortcut.TargetPath
                $looksLikeMain = (($name -match "Whisperwood|Enhanced Living") -or ($oldTarget -match "Whisperwood|Enhanced Living")) -and ($name -notmatch "Demo") -and ($oldTarget -notmatch "Demo")
                if ($looksLikeMain) {{
...
$timer.Add_Tick({{
    ...
    if ($script:installerProcess.HasExited) {{
        ...
        Repair-WhisperwoodShortcuts $target
