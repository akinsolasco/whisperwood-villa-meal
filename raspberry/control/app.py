...
app = FastAPI(title="Whisperwood Control Service", version=APP_VERSION)
...
def send_email_message(subject: str, body: str, recipients: list[str], settings: Any = None) -> dict[str, Any]:
    ...
    msg["Date"] = format_datetime(local_now())
    msg["X-Whisperwood-Timezone"] = f"{LOCAL_TIMEZONE_NAME}; {LOCAL_TIMEZONE_LABEL}"
...
def process_battery_email_alerts(devices: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    lines.extend([
        ...
        "",
        f"Generated by {platform.node()} at {readable_now()}.",
    ])
...
@app.get("/health")
def health():
    uptime_s = int((local_now() - STARTED_AT).total_seconds())
    return {
        "ok": True,
        "service": "control",
        "version": APP_VERSION,
        ...
        "uptime": f"{uptime_s}s",
        "uptime_s": uptime_s,
        "last_restart": STARTED_AT.isoformat(timespec="seconds"),
    }
...
def battery_alert_test_email(payload: BatteryTestEmailPayload, x_whisperwood_key: str | None = Header(default=None)):
    ...
    result = send_email_message(
        ...
        f"This is a test battery notification from Enhanced Living Whisperwood.\n\nIf you received this, Raspberry Pi email alerts are configured.\n\nSent at {readable_now()}.",
        recipients,
    )
...
def integration_settings_test_email(payload: BatteryTestEmailPayload, x_whisperwood_key: str | None = Header(default=None)):
    ...
    result = send_email_message(
        ...
        f"This is a test email from the Raspberry Pi Control Service.\n\nIf you received this, the saved SMTP settings are working.\n\nSent at {readable_now()}.",
        recipients,
        settings=settings,
    )
